...
import { getVendorCoordinates, getVendorPaymentMethodDetails, isVendorPresenceFresh, formatVendorPresenceAge } from '../lib/vendor'
...
export default function LegacyOrderTrackingPage() {
  const { id } = useParams()
  ...
  const vendorCoordinates = useMemo(() => {
    return isVendorPresenceFresh(vendor, presenceClock) ? getVendorCoordinates(vendor.location) : null
  }, [vendor, presenceClock])

  useEffect(() => {
    const timer = window.setInterval(() => setPresenceClock(Date.now()), 10000)
    return () => window.clearInterval(timer)
  }, [])

  const vendorHasLiveLocation = Boolean(vendorCoordinates)
  const vendorHasFallbackLocation = false
    ...
    <div className="min-h-screen bg-transparent">
      <div className="mx-auto max-w-6xl space-y-4 overflow-x-hidden px-3 py-5 sm:px-4 sm:py-6">
        ...
        <section className="grid min-w-0 grid-cols-3 gap-2 sm:gap-4">
          ...
          <div className="min-w-0 rounded-[18px] bg-white p-3 shadow-sm ring-1 ring-slate-200/80 sm:rounded-[24px] sm:p-4">
            <div className="text-[10px] font-semibold uppercase tracking-[0.12em] text-slate-400 sm:text-xs sm:tracking-[0.16em]">Lokasi</div>
            <div className="mt-2 break-words text-base font-semibold leading-tight text-slate-900 sm:text-lg">{vendorHasLiveLocation ? formatVendorPresenceAge(vendor) : 'Tidak aktif'}</div>
            <div className="mt-1 hidden text-xs text-slate-500 sm:block">
              Tidak ada estimasi waktu tiba otomatis.
            </div>
          </div>
        </section>
