...
import { getDisplayName } from '../lib/vendor'
...
function OrdersPanel({ currentUser, role }) {
  ...
  if (loading) {
    return <div className="rounded-2xl bg-white p-4 text-sm text-gray-500 shadow-sm ring-1 ring-gray-100">Memuat pesanan...</div>
  }
    ...
    <div className="rounded-2xl bg-white p-4 shadow-sm ring-1 ring-gray-100">
      <div className="mb-4 flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500">Pantau transaksi terbaru Anda di sini.</p>
        </div>
        <button onClick={fetchOrders} className="rounded-lg border border-gray-300 px-3 py-1 text-sm">
          Refresh
        </button>
      </div>
            ...
            <div key={order.id} className="rounded-2xl border border-gray-200 p-4">
              <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                <div>
                  ...
                  <div className="mt-1 whitespace-pre-wrap text-sm text-gray-600">{order.items || '-'}</div>
      ...
      <div className="flex items-center gap-4">
        <div className="flex h-16 w-16 items-center justify-center overflow-hidden rounded-full bg-gray-100">
          {profile.photo_url ? (
            ...
            <div className="text-xl font-semibold text-gray-500">{(profile.name || 'U')[0]}</div>
          )}
        </div>

        <div>
          <div className="font-semibold text-gray-900">{profile.name || profile.email}</div>
      ...
      <div className="mt-4">
        {!editing ? (
          <>
            ...
            <div className="mt-3">
              <button onClick={() => setEditing(true)} className="rounded-lg border border-gray-300 px-3 py-2 text-sm">
                Edit Profil
              </button>
            </div>
          </>
          ...
          <aside className="space-y-4">
            <div className="rounded-2xl bg-white p-4 shadow-sm ring-1 ring-gray-100">
              <div className="flex items-center gap-3">
                <div className="flex h-14 w-14 items-center justify-center overflow-hidden rounded-full bg-gray-100 text-xl">
                  {avatarUrl ? (
                    <img src={avatarUrl} alt="avatar" className="h-full w-full object-cover" />
                  ) : (
                    <div className="font-semibold text-gray-500">{displayName[0]}</div>
                  )}
                </div>
              ...
              <div className="mt-4 flex items-center gap-2">
                <span className={`rounded-full px-3 py-1 text-xs font-medium ${
                  isVendor ? 'bg-green-50 text-green-700' : 'bg-gray-100 text-gray-700'
                }`}>
            ...
            <nav className="rounded-2xl bg-white p-4 shadow-sm ring-1 ring-gray-100">
              ...
              <p className="mt-3 text-sm text-gray-500">Gunakan menu ini untuk berpindah antar fitur dengan cepat.</p>
            </nav>
          </aside>
