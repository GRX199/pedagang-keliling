...
select pg_temp.expect_error(format('select public.confirm_pickup_handover(%L)', :'self_id'), 'buyer cannot complete');
select set_config('request.jwt.claim.sub', '10000000-0000-0000-0000-000000000003', true);
select pg_temp.expect_error(format('select public.confirm_pickup_handover(%L)', :'self_id'), 'stranger cannot complete');
select set_config('request.jwt.claim.sub', '10000000-0000-0000-0000-000000000002', true);
select pg_temp.expect_error(format('select public.confirm_pickup_handover(%L)', :'self_id'), 'pending cannot complete');
update public.orders set status = 'accepted' where id = :'self_id';
update public.orders set status = 'ready' where id = :'self_id';
select pg_temp.expect_error(format('select public.confirm_pickup_handover(%L)', :'self_id'), 'unpaid cannot complete');
...
set local role authenticated;
select set_config('request.jwt.claim.sub', '10000000-0000-0000-0000-000000000002', true);
select pg_temp.expect_error(format('select public.confirm_pickup_handover(%L)', :'self_id'), 'blocked vendor cannot complete');
reset role;
select set_config('request.jwt.claim.sub', '', true);
update public.profiles set account_status = 'active' where id = '10000000-0000-0000-0000-000000000002';
set local role authenticated;
select set_config('request.jwt.claim.sub', '10000000-0000-0000-0000-000000000002', true);
select public.confirm_pickup_handover(:'self_id');
select public.confirm_pickup_handover(:'self_id');
select pg_temp.assert_true((select status = 'completed' and handed_over_at is not null from public.orders where id = :'self_id'), 'code-free handover recorded without collector name');
